...
-(void) saveGame
{
    ...
    [KOKPersistence saveObject: self.model withFileName:@"LightsOut_Model"];
...
-(void) loadGame
{
    ...
    self.model = [KOKPersistence loadObjectfromFileName:@"LightsOut_Model"];
...
-(void) createNewModel
{
   //NSLog(@"CreateNewModel got called, with size: %d", self.matrixSize);
    _model = [[KOKGame alloc] initWithBoardSize:self.matrixSize andSequenceLength:self.sequenceLength andColor:self.color];
}
